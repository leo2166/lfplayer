...
async function getOrphanKeys() {
    ...
    // 2. Get all blob_urls from Supabase
    const { data: songs, error: dbError } = await supabaseAdmin.from('songs').select('blob_url');
    if (dbError) {
    ...
    songs.forEach(song => {
        if (song.blob_url && song.blob_url.startsWith(r2PublicUrl)) {
            ...
            if (finalKey) supabaseFileKeys.add(finalKey);
        }
    });
